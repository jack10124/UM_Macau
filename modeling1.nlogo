...
to setup
  clear-all
  set-default-shape turtles "bacteria"
...
to setup-food  ;; patch procedure
  ...
  if (distancexy (0.6 * max-pxcor) 0) < 7
  [ set food-source-number 1 ]
  ;; setup food source two on the lower-left
  if (distancexy (-0.6 * max-pxcor) (-0.6 * max-pycor)) < 7
  [ set food-source-number 2 ]
  ;; setup food source three on the upper-left
  if (distancexy (-0.6 * max-pxcor) (0.8 * max-pycor)) < 9
...
to go  ;; forever button
  ...
  [ if who >= ticks [ stop ] ;; delay initial departure
    ...
    [ die ]       ;; carrying food? take it back to nest
    wiggle
    fd 1 ]
...
Circle -7500403 false false 99 129 42
Circle -1 true false 90 45 120
Circle -1 true false 75 165 88
